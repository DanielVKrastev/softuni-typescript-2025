...
function isUser(user: unknown): user is User {
    return user != undefined && typeof user === 'object' && 
        ...
        ((typeof user.passwordHash === 'string' && user.passwordHash.length === 20) || (Array.isArray(user.passwordHash) && user.passwordHash.length === 4 && user.passwordHash.every(x => typeof x === 'string' && x.length === 8))) &&
